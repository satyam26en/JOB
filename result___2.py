...
Original file is located at
    https://colab.research.google.com/drive/1QGziHKG3QV1E6eBYfbHG06ovyY3N7dcl
"""
...
# Function to get data for a specific experience type
def get_data_for_experience(experience_type):
    return pd.DataFrame([
        {'Job Role': role, 'Number of Openings': data[experience_type], 'Hover Text': create_hover_text(role)}
        for role, data in job_openings_data.items()
    ]).sort_values(by='Number of Openings')
...
# Select the appropriate data and y-axis scale
if experience_type == 'All':
    filtered_data = total_openings_by_role
    yaxis_dtick = 5000
else:
    filtered_data = get_data_for_experience(experience_type)
    yaxis_dtick = 500
